...
**🎨 Products View Redesigned** — View 3 has been reorganised from 5 loosely grouped columns into 4 logical stacks:

| Stack | Contents |
|---|---|
| **📋 Product Selection** | Product dropdowns + application method selectors together as the primary workflow |
| **📝 Product Details** | Split into 3 themed cards: 🌿 Nutrition (Kelp, Granular Fert, Liquid Fert), 🏗️ Soil & Growth (Humate, Soil Wetter, PGR), 🛡️ Protection (Pre-emergent, Insecticide, Fungicide, Herbicide) |
| **⚖️ Rates & Units** | Application rates and product units combined (were two separate columns) |
| **🧪 Enabled & Custom** | Product toggles + conditional custom product fields |

Each product detail block now shows the product name, rate, active ingredient, and notes in its own section with dividers — no more wall of text.

**🐛 v3.7.1 Critical Fix — Product Details working** — All 20 DB + product_info sensors were producing wrong entity_ids (e.g. `sensor.kelp_seaweed_db` instead of `sensor.lawn_db_kelp`). Sensors renamed and `unique_id` removed to prevent entity registry conflicts. Pre-emergent method key corrected (`Liquid Spray` → `Foliar Spray`), fungicide method key corrected (`Soil Drench` → `Curative Drench`).

**🎨 Styling** — Consistent section headers across all views (uppercase, letter-spacing, green underline border). Category icons added to rate/unit entity rows. Missing CSS variables added to Products view theme block.
...
| Template sensors | 33 | `lawn_gdd_today`, `lawn_spray_conditions`, `lawn_et_estimate`, `lawn_irrigation_minutes`, `lawn_avg_soil_moisture`, `lawn_low_stock_items`, `lawn_pgr_forecast_date`, 10× `lawn_db_*` (single-source product dicts), 10× `lawn_product_info_*` |
...
| Card | Purpose | Link |
|---|---|---|
| **ApexCharts Card** | Season Review dual-axis charts (View 4) | [GitHub](https://github.com/RomRider/apexcharts-card) |
...
The **Products** tab starts with a **🧪 Enabled Products** section. All 11 products are enabled by default. Toggle off any product you don't use — it will be hidden from the Dashboard task grid, removed from the Inventory stock list, and its reminder notifications will be suppressed. You can re-enable at any time.

### Set Your Product Units

In the **Rates & Units** stack is the **Product Units** card. Each product has a dropdown to choose its measurement unit: `ml`, `g`, `L`, or `kg`. This unit is used everywhere — application rate labels, stock levels, logbook entries, area breakdowns, and low-stock alert messages. Change it whenever you switch between liquid and granular formulations of a product. Defaults: `g` for Granular Fert, `ml` for everything else.

### Select Your Products

The **📋 Product Selection** stack has two cards:

1. **Selected Products** — Pick the commercial product you use in each category (e.g., "Astro 120 ME" for PGR, "LawnPride Kelpxtra" for Kelp). Selecting a product auto-fills the application rate, unit, and nitrogen fraction. Choose "Custom" in any category to set your own values manually.

2. **Application Method** — Seven categories with multiple application methods (e.g., Foliar vs Soil Drench) have a separate method dropdown. Changing the method recalculates the suggested rate.

The **📝 Product Details** stack below shows a live summary of every selected product grouped by theme — rate range, active ingredient, and application notes. Use this as a quick reference when mixing.
...
All product configuration organised into 4 stacks:

| Stack | Contents |
|---|---|
| **📋 Product Selection** | Product database dropdowns with category icons + application method selectors |
| **📝 Product Details** | Three themed cards — 🌿 Nutrition (Kelp, Granular, Liquid Fert), 🏗️ Soil & Growth (Humate, Soil Wetter, PGR), 🛡️ Protection (Pre-emergent, Insecticide, Fungicide, Herbicide). Per-product blocks with dividers showing rate, AI, and notes. |
| **⚖️ Rates & Units** | Application rates per 100m² and product unit selectors (ml/g/L/kg) |
| **🧪 Enabled & Custom** | Product enable/disable toggles, conditional custom product fields (appear when "Custom" selected), and built-in product guide |
...
Replace both `lawn_care.yaml` and `lovelace_dashboard.yaml`, then reload Template Entities (**Developer Tools → YAML → Reload Template Entities**).

**v3.8.0 includes the v3.7.1 critical fix** for entity_id mismatches. If you're coming from v3.7.0 directly, you may have orphaned entities from the old sensor names. After upgrading:

1. Go to **Settings → Devices & Services → Entities**
2. Filter for old names like `kelp_seaweed`, `granular_fertiliser`, `humate_humic`, `pre_emergent_herbicide`, `plant_growth_regulator`
3. Delete any stale entities you find
4. Restart HA if Product Details still shows "unknown"

The Products view dashboard layout has changed significantly — this will apply automatically when you paste the new dashboard YAML.
...
|---|---|---|
| `input_select.lawn_product_db_*` | 10 | First product in each category (e.g., "LawnPride Kelpxtra" for Kelp) |
...
After restart, go to **Products → Product Selection** to select your actual products. The auto-fill automation will set the rate, unit, and N-fraction automatically. Override any values manually if needed.
...
After restart: disable unused products in **Enabled Products**, select your products in **Product Selection**, and verify units in **Rates & Units**.
...
- Products view (View 3) redesigned — 4 logical stacks: Product Selection, Product Details (Nutrition / Soil & Growth / Protection), Rates & Units, Enabled Products & Custom
- Consistent section header styling across all views (uppercase, letter-spacing, green underline)
- Added category icons to rate/unit entity rows
- Added missing CSS variables to Products view theme block

### v3.7.1

- **CRITICAL FIX:** Product Details showing `unknown` / `None` for all categories. Root cause: sensor names produced wrong entity_ids (e.g. `"Kelp / Seaweed DB"` → `sensor.kelp_seaweed_db` instead of expected `sensor.lawn_db_kelp`). Renamed all 20 DB + product_info sensors and removed `unique_id` to prevent entity registry conflicts.
...
Each category's product dictionary lives in **one place**: the `products` attribute of the corresponding DB sensor (e.g. `Lawn DB Kelp`). To add a product, edit `lawn_care.yaml` in two places:
...
**Step 2** — Add the product entry to the `products` attribute dict in the corresponding DB sensor:
...
**Product Details shows "unknown" / "None"** — This was fixed in v3.7.1/v3.8.0. If you still see this after upgrading, go to **Settings → Devices & Services → Entities**, filter for old sensor names like `kelp_seaweed_db` or `granular_fertiliser_product_info`, delete them, then reload Template Entities or restart HA.
